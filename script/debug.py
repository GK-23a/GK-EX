...
with open('data/data.json', encoding='UTF-8') as jsonfile:
    character_data = json.loads(jsonfile.read())['character_data']
wlog(__file__, 'out/debug_.log', '"data.json"读取完成。', 'Debug')
...
for ch_id in debug_character_data:
    ch_img = character_card.cardbuild(ch_id, debug_character_data, Qt = False)
    if ch_img:
